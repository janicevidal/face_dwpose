...
class IntegralPoseEstimatorDistiller(BaseModel, metaclass=ABCMeta):
    ...
    def loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        ...
        with torch.no_grad():
            ...
            coords, sigmas, lt_x, lt_y = self.teacher.head(fea_t)
            pred_t = (lt_x, lt_y)
        ...
        if 'loss_logit' in all_keys:
            ...
            # losses[loss_name] = self.distill_losses[loss_name](pred, pred_t, self.student.head.multi_losses['loss_simc'].beta, target_weight)
            losses[loss_name] = self.distill_losses[loss_name](pred, pred_t, 10, target_weight)
            if not self.two_dis:
                losses[loss_name] = (1-self.epoch/self.max_epochs)*losses[loss_name]
    ...
    def head_loss(
        ...
        coords, sigmas, pred_x, pred_y = self.student.head.forward(feats)
        ...
        if self.student.head.loss_cfg is not None:
            gt = torch.cat([
                d.gt_instance_labels.keypoint_labels for d in batch_data_samples
            ],
                            dim=0)
            
            all_keys = self.student.head.multi_losses.keys()
    
            if 'loss_l1' in all_keys:
                loss_name = 'loss_l1'
                losses[loss_name] = self.student.head.multi_losses[loss_name](coords, gt, keypoint_weights)
            
            if 'loss_adl' in all_keys:
                loss_name = 'loss_adl'
                losses[loss_name] = self.student.head.multi_losses[loss_name](coords, gt)
            
            if 'loss_rle' in all_keys:
                loss_name = 'loss_rle'
                losses[loss_name] = self.student.head.multi_losses[loss_name](coords, sigmas, gt, keypoint_weights)
            
            if 'loss_simc' in all_keys:
                loss_name = 'loss_simc'
                losses[loss_name] = self.student.head.multi_losses[loss_name](pred_simcc, gt_simcc, keypoint_weights)

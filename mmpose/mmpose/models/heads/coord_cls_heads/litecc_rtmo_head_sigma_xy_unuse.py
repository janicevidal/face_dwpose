...
class LiteMLEHead(BaseHead):
    ...
    def __init__(
        ...
        if self.sigma_xy:
            self.sigma_fc = nn.Sequential(nn.Linear(hidden_dims, 2), nn.Sigmoid(), Scale(0.1))
        else:
            self.sigma_fc = nn.Sequential(nn.Linear(hidden_dims, 1), nn.Sigmoid(), Scale(0.1))
    ...
    def generate_target_heatmap(self, kpt_targets, sigmas):
        ...
        # normalize
        sigmas_ = sigmas.clip(min=1e-3)
        
        if sigmas_.shape[-1] == 2:
            sigmas_x = sigmas_.narrow(2, 0, 1)
            sigmas_y = sigmas_.narrow(2, 1, 1)
        else:
            sigmas_x = sigmas_
            sigmas_y = sigmas_

        dist_x = dist_x / sigmas_x
        dist_y = dist_y / sigmas_y

        hm_x = torch.exp(-dist_x / 2) / sigmas_x
        hm_y = torch.exp(-dist_y / 2) / sigmas_y

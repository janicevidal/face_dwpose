...
class MultiTaskHybridLiteMLEHead(BaseHead):
    ...
    def __init__(
        ...
        self.scale_norm_std = scale_norm_std
        self.beta = beta
        ...
        # with_sihn / with_sihn_relu True时，ScaleNorm中的 self.g 实际上不起作用，训练输出值异常大，会导致部署阶段 fp16 模型推理失败，标准模式另写为 ScaleNormSTD
        if scale_norm:
            self.norm = ScaleNorm(self.flatten_dims)
        if scale_norm_std:
            self.norm = ScaleNormSTD(self.flatten_dims)
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        if self.scale_norm or self.scale_norm_std:

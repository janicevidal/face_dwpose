...
class MultiTaskHybridLiteMLEHead(BaseHead):
    ...
    def __init__(
        ...
        # mnn convert fc to conv
        self.yaw_conv = nn.Conv2d(euler_in_channels, euler_bin[0]*2 // euler_bin_interval[0], kernel_size=1, stride=1)
        self.pitch_conv = nn.Conv2d(euler_in_channels, euler_bin[1]*2 // euler_bin_interval[0], kernel_size=1, stride=1)
        self.yaw_conv.weight.data = self.yaw_fc.weight.data.view(euler_bin[0]*2 // euler_bin_interval[0], euler_in_channels, 1, 1)
        self.yaw_conv.bias.data = self.yaw_fc.bias.data
        self.pitch_conv.weight.data = self.pitch_fc.weight.data.view(euler_bin[1]*2 // euler_bin_interval[0], euler_in_channels, 1, 1)
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        if torch.onnx.is_in_onnx_export():        
            ...
            yaw = self.yaw_conv(euler_feature_conv).reshape(1, self.yaw_fc.out_features)
            pitch = self.pitch_conv(euler_feature_conv).reshape(1, self.pitch_fc.out_features)
        else:

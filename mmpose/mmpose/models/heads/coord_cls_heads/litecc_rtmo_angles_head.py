...
class MultiTaskLiteMLEHead(BaseHead):
    """
    Args:
    ...
    def __init__(
        ...
        self.cls_x_conv = nn.Conv2d(hidden_dims, W, kernel_size=1, bias=False)
        self.cls_y_conv = nn.Conv2d(hidden_dims, H, kernel_size=1, bias=False)
        self.cls_x_conv.weight.data = self.cls_x.weight.data.view(W, hidden_dims, 1, 1)
        self.cls_y_conv.weight.data = self.cls_y.weight.data.view(H, hidden_dims, 1, 1)
        self.cls_x_conv.requires_grad_(False)
        self.cls_y_conv.requires_grad_(False)
        ...
        # euler MLP
        assert euler_bin_interval in [1, 2], 'Only support interval 1 and 2'
        
        if euler_bin_interval==1:
            yaw_out_channels = euler_bin[0]*2
            pitch_out_channels = euler_bin[1]*2
        elif euler_bin_interval==2:
            yaw_out_channels = euler_bin[0]
            pitch_out_channels = euler_bin[1]
            
        self.yaw_fc = nn.Linear(euler_in_channels, yaw_out_channels)
        self.pitch_fc = nn.Linear(euler_in_channels, pitch_out_channels)
        
        # mnn convert fc to conv
        self.yaw_conv = nn.Conv2d(euler_in_channels, yaw_out_channels, kernel_size=1, stride=1)
        self.pitch_conv = nn.Conv2d(euler_in_channels, pitch_out_channels, kernel_size=1, stride=1)
        self.yaw_conv.weight.data = self.yaw_fc.weight.data.view(yaw_out_channels, euler_in_channels, 1, 1)
        self.yaw_conv.bias.data = self.yaw_fc.bias.data
        self.pitch_conv.weight.data = self.pitch_fc.weight.data.view(pitch_out_channels, euler_in_channels, 1, 1)
        self.pitch_conv.bias.data = self.pitch_fc.bias.data
        self.yaw_conv.requires_grad_(False)
        self.pitch_conv.requires_grad_(False)
        
        self.linspace_yaw = torch.arange(-euler_bin[0] * 1.0, euler_bin[0]* 1.0, euler_bin_interval).reshape(1, yaw_out_channels)
        self.linspace_pitch = torch.arange(-euler_bin[1]* 1.0, euler_bin[1]* 1.0, euler_bin_interval).reshape(1, pitch_out_channels)
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
        """Forward the network.
        ...
        if torch.onnx.is_in_onnx_export():        
            euler_feature = self.avg(feats[0])
            yaw = self.yaw_conv(euler_feature).reshape(1, self.yaw_fc.out_features)
            pitch = self.pitch_conv(euler_feature).reshape(1, self.pitch_fc.out_features)
        else:
        ...
        if torch.onnx.is_in_onnx_export(): 
            ...
            pred_x = self.cls_x_conv(feats_).reshape(1, self.out_channels, self.cls_x.out_features)
            pred_y = self.cls_y_conv(feats_).reshape(1, self.out_channels, self.cls_y.out_features)
        else:   
        ...
        if torch.onnx.is_in_onnx_export():
            ...
            return simc_pred, sigmas, pred_x_sfm, pred_y_sfm, yaw, pitch, pred_yaw, pred_pitch
    ...
    def predict(self,
        ...
        if test_cfg.get('flip_test', False):
            ...
            _batch_coords, _,  _, _, _, _, pred_yaw, pred_pitch = self.forward(_feats)

            _batch_coords_flip, _, _, _, _, _, _, _ = self.forward(
                _feats_flip)
            ...
            batch_coords, _, _, _, _, _, pred_yaw, pred_pitch = self.forward(feats)  # (B, K, D)
    ...
    def loss(
        ...
        coords, sigmas, pred_x, pred_y, yaw, pitch, pred_yaw, pred_pitch = self.forward(feats)
        ...
        if self.loss_cfg is not None:
            ...
            if 'loss_angle_ce' in all_keys:
                ...
                losses['loss_ce_yaw'] = self.multi_losses[loss_name](yaw, gt_euler_labels[:, 0])
                losses['loss_ce_pitch'] = self.multi_losses[loss_name](pitch, gt_euler_labels[:, -1])

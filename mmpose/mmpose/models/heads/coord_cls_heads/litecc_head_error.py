...
class LiteCCHead(BaseHead):
    ...
    def __init__(
        ...
        self.with_dsntrle = with_dsntrle
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
        ...
        if self.with_dsntrle: # rle
            ...
            return pred_x, pred_y, coords, heatmaps
            
        return pred_x, pred_y, torch.zeros(1, 1), torch.zeros(1, 1)
    ...
    def predict(
        ...
        if test_cfg.get('flip_test', False):
            ...
            _batch_pred_x_flip, _batch_pred_y_flip, _, _ = self.forward(_feats_flip)
    ...
    def loss(
        ...
        pred_x, pred_y, coords, heatmaps = self.forward(feats)
        ...
        # calculate losses
        losses = dict()
        if not self.with_dsntrle and self.loss_module is not None:        
            loss = self.loss_module(pred_simcc, gt_simcc, keypoint_weights)
            losses.update(loss_kpt=loss)
        
        if self.with_dsntrle and self.loss_cfg is not None:
            gt = torch.cat([
            ...
            if 'loss_dsnt' in all_keys:
                loss_name = 'loss_dsnt'
                losses[loss_name] = self.multi_losses[loss_name](coords[...,:2], gt, heatmaps, keypoint_weights)

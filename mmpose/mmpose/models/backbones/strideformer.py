...
class StrideFormer(BaseBackbone):
    ...
    def __init__(
        ...
        for i in range(len(depths)):
            ...
            trans = BasicLayer(
                ...
                drop_path=dpr,
                act_cfg=act_cfg,
                stride_attention=stride_attention[i],
            )
        ...
        elif self.inj_type == 'AAMSx16':
            self.inj_module = InjectionMultiSumallmultiallsumSimpx16(
                in_channels=out_feat_chs, out_channels=out_channels)
            self.feat_channels = [
                out_channels,
            ]
...
class InjectionMultiSumallmultiallsumSimpx16(nn.Module):
    """the Aggregate Attention Module.
    ...
    def forward(self, inputs):
        # x_x16, x_x32
        low_feat1 = self.embedding(inputs[1])

        high_feat_act = F.interpolate(
            self.act_embedding(inputs[2]),
            size=low_feat1.shape[2:],
            mode='bilinear',
        )
        high_feat = F.interpolate(
            self.embedding_2(inputs[2]),
            size=low_feat1.shape[2:],
            mode='bilinear')

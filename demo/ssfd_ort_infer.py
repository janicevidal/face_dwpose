...
class SSFD:
    ...
    def forward(self, img, thresh):
        ...
        for idx, stride in enumerate(self._feat_stride_fpn):
            ...
            scores = (net_outs[idx].transpose(0, 2, 3, 1)).reshape(1, K * self._num_anchors, 1)[0]
            bbox_preds = (net_outs[idx + fmc].transpose(0, 2, 3, 1)).reshape(1, K * self._num_anchors, 4)[0]
            bbox_preds = bbox_preds * stride
            
            if self.use_kps:
                kps_preds =(net_outs[idx + fmc * 2].transpose(0, 2, 3, 1)).reshape(1, K * self._num_anchors, 10)[0] * stride
...
if __name__ == '__main__':
    # detector = SSFD(
    #     model_file=
    #     'models/scrfd_lite_ghost_m2_256_finetune_kpts_tal_resume_shape160x256.onnx')
